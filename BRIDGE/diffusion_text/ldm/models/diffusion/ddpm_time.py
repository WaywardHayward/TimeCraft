...
class ConditioningMLP(nn.Module):
    ...
    def forward(self, c, text_embedding):
        fused_input = torch.cat([c, text_embedding], dim=-1)  
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.model = DiffusionWrapper(unet_config, conditioning_key, text_embedding_dim)
        count_params(self.model, verbose=True)
        self.conditioning_mlp = ConditioningMLP(input_dim=64+4096, hidden_dim=512, output_dim=64)
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, return_mask=False, text_embedding=None):
        ...
        if text_embedding is not None:
            text_embedding = text_embedding.to(c.device)  

            if c is None:
                c = text_embedding  
            else:
                c = self.conditioning_mlp(c, text_embedding)
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key, text_embedding_dim=None):
        super().__init__()
    ...
    def forward(self, x, t, sampled_concept=None, sampled_index=None, c_concat: list=None, c_crossattn: list=None, sub_scale: list=None, cfg_scale=1., cond_drop_prob=0., cond_part_drop=False, mask=None, text_embedding=None):
        ...
        if text_embedding is not None and self.text_embedding_dim is not None:
            text_embedding = text_embedding.view(-1, self.text_embedding_dim)
        
        out = self.diffusion_model(x, t, context=cc, mask=mask, cond_drop_prob=cond_drop_prob, cond_part_drop=cond_part_drop, text_embedding=text_embedding)
        
        return out
        
    def cfg_forward(self, x, t, sampled_concept=None, sampled_index=None, c_concat: list=None, c_crossattn: list=None, sub_scale: list=None, cfg_scale=1., mask=None, text_embedding=None):
        ...
        out = self.diffusion_model(x, t, context=cc, mask=mask, text_embedding=text_embedding)
