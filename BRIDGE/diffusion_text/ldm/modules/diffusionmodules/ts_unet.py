...
class UNetModel(nn.Module):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        AttentionBlock(
                            ...
                            ch, num_heads, dim_head, context_dim=context_dim, use_pam=self.use_pam, depth=transformer_depth
                        )
                    )
    ...
    def _forward(self, x, timesteps=None, text_embedding=None, context=None, mask=None, y=None, cond_drop_prob=0, **kwargs):
        """
        ...
        if text_embedding is not None and context_emb is not None:
            weight = 1  
            fusion_input = th.cat([context_emb, text_embedding.unsqueeze(1) * weight], dim=-1) 
            fusion_mlp = th.nn.Sequential(
                th.nn.Linear(fusion_input.shape[-1], context_emb.shape[-1]),
                th.nn.ReLU(),
                th.nn.Linear(context_emb.shape[-1], context_emb.shape[-1])
            ).to(device)
            context_emb = fusion_mlp(fusion_input)        
        hs = []
    ...
    def forward(self, x, timesteps=None, context=None, text_embedding=None, mask=None, y=None, cond_drop_prob=0, cond_part_drop=False, **kwargs):
        out = self._forward(x, timesteps, context, text_embedding, mask, y, cond_drop_prob, cond_part_drop=cond_part_drop, **kwargs)
        return out
    
    def forward_with_cfg(self, x, timesteps=None, context=None, text_embedding=None, y=None, cfg_scale=1,**kwargs):
        model_out = self._forward(x=x, timesteps=timesteps, context=context, text_embedding=text_embedding, y=y, cond_drop_prob=0.,**kwargs)
        if cfg_scale == 1:
            return model_out
        
        null_context_out = self._forward(x=x, timesteps=timesteps, context=context, text_embedding=text_embedding, y=y, cond_drop_prob=1.,**kwargs)
