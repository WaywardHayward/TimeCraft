...
def test_model_with_dp(model, data, trainer, opt, logdir):
    ...
    seq_len = data.window
    num_dp = 100  # Number of samples for constructing domain prompts
    all_metrics = {}

    for dataset in data.norm_train_dict:
        dataset_data = TSGDataset({dataset: data.norm_train_dict[dataset]},
                                  text_data_dict={dataset: data.norm_text_train_dict[dataset]})

        dataset_samples = []
        text_samples = []

        for idx in np.random.randint(dataset_data.__len__(), size=num_dp):
            sample = dataset_data.__getitem__(idx)
            dataset_samples.append(sample['context'])
            text_samples.append(sample['text_embedding']) 

        dataset_samples = np.vstack(dataset_samples)
        text_samples = np.vstack(text_samples)

        x = torch.tensor(dataset_samples).to('cuda').float().unsqueeze(1)[:num_dp]  
        text_emb = torch.tensor(text_samples).to('cuda').float()[:num_dp]  

        c, mask = model.get_learned_conditioning(x, return_mask=True, text_embedding=text_emb)  
        ...
        text_repeat = torch.repeat_interleave(text_emb, repeats, dim=0) 

        all_gen = []
        for _ in range(5 if not opt.debug else 1):  # Iterate to reduce maximum memory usage
            samples, _ = model.sample_log(
                cond=cond, batch_size=1000 if not opt.debug else 100,
                ddim=False, cfg_scale=1, mask=mask_repeat, text_embedding=text_repeat  # 
            )
